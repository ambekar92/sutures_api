...
class Rework_analysis_yearly {
  ...
  $query = "SELECT MONTH(js.updated_at) as _month, DATE_FORMAT(js.updated_at, '%b''%y') as month_desc,sum(ord_qty) as tot_batch_qty,sum(ord_qty/12) as tot_batch_qty_doz,SUM(A.tot_apprvd_qty) as tot_apprvd_qty,SUM(A.tot_apprvd_qty/12) as tot_apprvd_qty_doz,((SUM(A.tot_apprvd_qty)/(sum(ord_qty))*100))as tot_apprvd_qty_per,SUM(A.tot_rew_qty) as tot_rew,SUM(A.tot_rew_qty/12) as tot_rew_doz,((SUM(A.tot_rew_qty)/sum(ord_qty))*100)as tot_rew_per,COUNT(jc.batch_no) as tot_batch_crd_issued,
