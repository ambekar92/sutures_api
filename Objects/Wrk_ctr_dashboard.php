...
class Wrk_ctr_dashboard{
 ...
 function read(){
            ...
            left join (SELECT (case when tb_m_jobcard.urgent = 1 then Count(tb_m_jobcard.batch_no) end) as u_pending_cards,
                      (case when tb_m_jobcard.urgent = 0 then Count(tb_m_jobcard.batch_no) end) as R_pending_cards,
                       to_dept from tb_t_job_status join tb_m_jobcard on tb_t_job_status.batch_no = tb_m_jobcard.batch_no  group by to_dept)                          jp on jp.to_dept = D.wrk_ctr_code 
            left join (SELECT (case when tb_m_jobcard.urgent = 1 then Count(tb_m_jobcard.batch_no) end) as u_completed_cards,
                      (case when tb_m_jobcard.urgent = 0 then Count(tb_m_jobcard.batch_no) end) as R_completed_cards,present_dept  FROM `tb_t_job_card_trans` join tb_m_jobcard on tb_m_jobcard.batch_no = tb_t_job_card_trans.batch_no where status_code = 803 and oper_status = 807  and  date(tb_t_job_card_trans.updated_at) = '$date' group by present_dept) A on A.present_dept = D.wrk_ctr_code
...
group by D.wrk_ctr_code";
                ...
                $query = "SELECT M.mach_code,WC.wrk_ctr_code,WC.wrk_ctr_desc,M.mach_desc,ifnull(js.batch_no,'No Batch is Running') as batch_no,ifnull(jb.fg_code,'-')as fg_code,if((js.status_code != 803 and js.oper_status != 807),U.name,'-') as operator,if(me.on_off_status = 1 or me.on_off_status is null,1,0) as on_off,ifnull(ROUND((B.OK_Qnty)/12,2),0) as ok_qty,ifnull(ROUND((B.Reject_Qnty)/12,2),0) as rej_qty from tb_m_machine M 
 ...
 function read1(){
   ...
   function read2(){
    ...
    $query2 = "SELECT count(batch_no)as completed_cards,present_dept,ifnull(A.daily_target,0) as daily_target FROM tb_t_job_card_trans
    LEFT JOIN  (SELECT work_ctr_code,process,daily_target from  tb_t_prod_dash_h where date_ = '$date') A on tb_t_job_card_trans.present_dept = A.work_ctr_code 
    where status_code = 803 and oper_status = 807 and  date(tb_t_job_card_trans.updated_at) = '$date' group by present_dept";
