...
class Operator_jobcards {
    ...
    function read(){
     ...
     $query = "SELECT q.batch_no,A.wrk_ctr_desc as DEPT,q.wrk_ctr_code as DEPT_code,ifnull(q.qty,0) as OK_QTY,ifnull(r.qty,0) as REJ_QTY,mc.mach_desc as MACHINE_NAME,date_format(A.updated_at,'%d-%m-%Y %H:%i:%s') as  TIME_FROM,date_format(B.updated_at,'%d-%m-%Y %H:%i:%s') as TIME_TO,Time(ABS(timediff(B.updated_at,A.updated_at))) as duration,SEC_TO_TIME(tb_m_fg_opr.bnch_mrk_prod_time * 60) as target_time,(((SEC_TO_TIME(tb_m_fg_opr.bnch_mrk_prod_time * 60)/timediff(B.updated_at,A.updated_at)))*100) as efficiency FROM  `tb_t_job_card_trans` A 
     ...
     join tb_m_fg_opr on tb_m_fg_opr.fg_code = tb_m_jobcard.fg_code and A.present_dept
