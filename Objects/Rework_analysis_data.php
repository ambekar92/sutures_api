...
class Rework_analysis_data {
    ...
    function read(){
        ...
        $sql1= "SELECT tb_m_qlty_code.wrk_ctr_code,tb_m_qlty_code.qlty_code,tb_o_workcenter.wrk_ctr_desc,tb_m_qlty_code.qlty_code_desc FROM `tb_m_qlty_code` join tb_o_workcenter on tb_m_qlty_code.wrk_ctr_code = tb_o_workcenter.wrk_ctr_code where tb_m_qlty_code.qlty_type_code = 501";
...
$query = "SELECT max(date(ph.updated_at))as datee,jc.batch_no,jc.fg_code,fg.type,jc.ord_qty as st_qty,MAX(IF(ph.wrk_ctr_code = 103021 and ph.qlty_code = 620,(ph.qty),' - '))apprvd_qty,
   IFNULL(SUM(rew.qty),0)tot_rew,
    MAX(IF(ph.wrk_ctr_code = 103020,(rew.qty),' - '))final_insp_rew,round(((MAX(IF(ph.wrk_ctr_code = 103021 and ph.qlty_code = 620,(ph.qty),' - '))/(jc.ord_qty))*100),2) as yield, ".$str." 
