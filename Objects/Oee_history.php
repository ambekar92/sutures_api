...
ORDER BY `tb_t_oee`.`date_`  DESC ";
   ...
   WHERE date_ > DATE_SUB('$date', INTERVAL 11 MONTH) AND date_ <= '$date' GROUP BY tb_o_workcenter.wrk_ctr_code,_month";
    ...
    $query = "SELECT o.date_,o.wrk_ctr_code,tb_o_workcenter.wrk_ctr_desc,o.mach_code,tb_m_machine.mach_desc,SEC_TO_Time(o.plnd_prod_time)as plnd_prod_time,SEC_TO_Time(o.run_time)as run_time,SEC_TO_Time(o.idle_time)as idle_time,o.target_prod,actual_prod,round((o.total_count/12),2)as total_count,round((o.ok_qty/12),2)as ok_qty,round((o.rej_qty/12),2)as rej_qty,o.availability_perc,o.performance_perc,o.quality_perc,o.oee_perc FROM `tb_t_oee` o
...
where o.mach_code = '$mach_code' and date_ > DATE_SUB('$date', INTERVAL 30 DAY) AND date_ <= '$date'";
    ...
    $query = "SELECT concat('WEEK ',WEEK(date_,0),' ( ',DATE_FORMAT(MIN(date_),'%d-%m-%Y'),' - ',DATE_FORMAT(MAX(date_),'%d-%m-%Y'),' )') as weekly,WEEK(date_,0) as weeks,o.wrk_ctr_code,tb_o_workcenter.wrk_ctr_desc,o.mach_code,tb_m_machine.mach_desc,SEC_TO_Time(SUM(o.plnd_prod_time)) as plnd_prod_time ,SEC_TO_Time(SUM(o.run_time)) as run_time,SEC_TO_Time(SUM(o.idle_time))as idle_time,sum(o.target_prod) as target_prod,sum(actual_prod) as actual_prod,round((SUM(o.total_count)/12),2) as total_count, round((SUM(o.ok_qty)/12),2) as ok_qty,round((SUM(o.rej_qty)/12),2) as rej_qty,o.availability_perc,o.performance_perc,o.quality_perc,o.oee_perc FROM `tb_t_oee` o
    left OUTER join tb_t_job_status js on js.to_dept = o.wrk_ctr_code and js.to_mach = o.mach_code and js.status_code = 802 and js.oper_status=806
    left join users U on js.emp_id = U.emp_id
    left join tb_m_jobcard jb on js.batch_no = jb.batch_no
    join tb_o_workcenter on tb_o_workcenter.wrk_ctr_code = o.wrk_ctr_code
...
where o.mach_code = '$mach_code' and date_ > DATE_SUB('$date', INTERVAL 4 WEEK) AND date_ <= '$date' GROUP by weeks";
    ...
    $query = "SELECT MONTH(date_) as _month, DATE_FORMAT(date_, '%b''%y') as month_desc,o.wrk_ctr_code,tb_o_workcenter.wrk_ctr_desc,o.mach_code,tb_m_machine.mach_desc,SEC_TO_Time(SUM(o.plnd_prod_time)) as plnd_prod_time ,SEC_TO_Time(SUM(o.run_time)) as run_time,SEC_TO_Time(SUM(o.idle_time))as idle_time,sum(o.target_prod) as target_prod,sum(actual_prod) as actual_prod,round((SUM(o.total_count)/12),2) as total_count, round((SUM(o.ok_qty)/12),2) as ok_qty,round((SUM(o.rej_qty)/12),2) as rej_qty,o.availability_perc,o.performance_perc,o.quality_perc,o.oee_perc FROM `tb_t_oee` o
    left OUTER join tb_t_job_status js on js.to_dept = o.wrk_ctr_code and js.to_mach = o.mach_code and js.status_code = 802 and js.oper_status=806
    left join users U on js.emp_id = U.emp_id
    left join tb_m_jobcard jb on js.batch_no = jb.batch_no
    join tb_o_workcenter on tb_o_workcenter.wrk_ctr_code = o.wrk_ctr_code
