...
class ageing{
    ...
    // select all query
    $query = "SELECT js.batch_no,jc.fg_code,js.to_dept_desc as dept_desc,if(js.to_dept = A.present_dept,IFNULL(U1.name,'NOT ASSIGNED'),'YET TO START') as operator,if(js.to_dept = A.present_dept,IFNULL(U2.name,'NOT ASSIGNED'),'YET TO START')as team_lead,date_format(jc.updated_at,'%d-%m-%Y %H:%i:%s')as created_on,date_format(js.updated_at,'%d-%m-%Y %H:%i:%s')as idle_from,DATEDIFF(CURRENT_DATE,jc.updated_at) as jobcard_cyl_days,DATEDIFF(CURRENT_DATE,js.updated_at) as no_of_idle_days FROM tb_t_job_status js 
