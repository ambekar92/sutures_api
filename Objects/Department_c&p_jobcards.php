...
class Department_cp_jobcards{
 ...
 function read(){
             ...
             $c_p_status = $data['c_p_status'];
              ...
              $query = "SELECT a.wrk_ctr_code,tb_t_job_card_trans.batch_no,tb_m_jobcard.fg_code,tb_m_jobcard.cust_name,tb_m_jobcard.plan,tb_m_jobcard.req_date,              if(tb_m_jobcard.urgent = 0,'REGULAR','URGENT')as type,a.qty as ok_qty,ifnull(sum(b.qty),0) as reject_qty,  date_format(tb_t_job_card_trans.updated_at,'%d-%m-%Y %H:%i:%s')as updated_at FROM `tb_t_job_card_trans` 
              ...
              where present_dept = '$wrk_ctr_code' and status_code = 803 and oper_status = 807 GROUP by batch_no order by tb_t_job_card_trans.updated_at DESC";
              }else{
              $query = "SELECT tb_t_job_status.batch_no,tb_m_jobcard.fg_code,tb_m_jobcard.cust_name,tb_m_jobcard.plan,tb_m_jobcard.req_date,if   (tb_m_jobcard.urgent = 0,'REGULAR','URGENT')as type,date_format(tb_t_job_status.updated_at,'%d-%m-%Y %H:%i:%s') updated_at  from tb_t_job_status join tb_m_jobcard on          tb_t_job_status.batch_no = tb_m_jobcard.batch_no
              where to_dept = '$wrk_ctr_code' order by tb_t_job_status.updated_at ASC ";
              }
