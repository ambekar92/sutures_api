...
//      JOIN   `tb_t_job_card_trans` B ON  A.oper_status = 806 and B.oper_status = 807 AND A.batch_no=B.batch_no AND A.present_mach = B.present_mach AND A.emp_id=B.emp_id AND A.sl_no<B.sl_no where A.emp_id = '$emp_id' GROUP BY date(A.updated_at)) dur  on dur.emp_id = em.emp_id where em.role_code = '101'";
    ...
    $query = "SELECT ifnull(date_format(dur.datee,'%d-%m-%Y'),date_format('$to_date','%d-%m-%Y'))as datee,ifnull(dur.datee,'$to_date') as date_c,em.emp_id,em.frst_name,ifnull(count(cnt.batch_no),0)as no_of_cards,ifnull(COUNT(DISTINCT(dur.present_mach)),0)as mach,ifnull(sec_to_time(SUM(time_to_sec(actual_duration))),'00:00:00')as actual_duration,'08:00:00' as target_duration,round(ifnull(((SUM(time_to_sec(actual_duration))/28800)*100),0),2) as t_eff,round(ifnull(((avg(cnt.bnch_mrk_prod_time)*ifnull(count(cnt.batch_no),0))/(SUM(time_to_sec(actual_duration))/60)*100),0),2)as w_eff from tb_m_employee em
    join (SELECT A.batch_no,timediff(B.updated_at,A.updated_at)as actual_duration,date(A.updated_at)as datee,A.emp_id,A.sl_no,A.present_mach FROM `tb_t_job_card_trans` A join tb_t_job_card_trans B on A.status_code = 802 and A.oper_status = 806 and B.oper_status = 807 and A.emp_id = B.emp_id and A.batch_no = B.batch_no and A.sl_no < B.sl_no and date(A.updated_at) = date(B.updated_at) where A.emp_id = '$emp_id' and date(A.updated_at) between  '$from_date' AND '$to_date'  GROUP BY A.sl_no,A.batch_no) dur  on dur.emp_id = em.emp_id 
