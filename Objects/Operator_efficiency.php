...
if ($emp_id == "NULL" AND $date != "NULL" ){
  ...
  JOIN   `tb_t_job_card_trans` B ON  A.oper_status = 806 and B.oper_status = 807 AND A.batch_no=B.batch_no AND A.present_mach = B.present_mach AND A.emp_id=B.emp_id AND A.sl_no<B.sl_no where date(A.updated_at) = '$date' GROUP BY A.emp_id,date(A.updated_at),date(B.updated_at) ) dur  on dur.emp_id = em.emp_id where em.role_code = '101' group by em.emp_id";
