...
class Prod_data_sheet {
    ...
    function read(){
        ...
        $query = "SELECT A.wrk_ctr_desc as DEPT,q.wrk_ctr_code as DEPT_CODE,date_format(B.updated_at,'%d-%m-%Y') as DATEE,u.name as OPERATOR,q.qty as OK_QTY,ifnull(r.qty,0) as REJ_QTY,mc.mach_desc as MACHINE_NAME,time(A.updated_at)as TIME_FROM,time(B.updated_at) as TIME_TO,Time(ABS(timediff(B.updated_at,A.updated_at))) as duration FROM  `tb_t_job_card_trans` A 
