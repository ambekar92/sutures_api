...
if ($type == 'NULL' AND $size_type == 'NULL') {
   $query = " SELECT sum(ord_qty) as tot_batch_qty,SUM(A.tot_apprvd_qty) as tot_apprvd_qty,SUM(A.tot_rej_qty) as tot_rej, COUNT(jc.batch_no) as tot_batch_crd_issued,(SUM(A.tot_apprvd_qty)/SUM(ord_qty)*100) as avg_yield,IFNULL(SUM(CASE WHEN fg.type = 'RB' THEN (ord_qty) END),0)RB_tot_batch_qty,IFNULL(SUM(CASE WHEN fg.type = 'RB' THEN (A.tot_apprvd_qty) END),0)RB_tot_apprvd_qty,IFNULL(SUM(CASE WHEN fg.type = 'RB' THEN (A.tot_rej_qty) END),0)RB_tot_rej,IFNULL(SUM(CASE WHEN fg.type = 'CT' THEN (ord_qty) END),0)CT_tot_batch_qty,IFNULL(SUM(CASE WHEN fg.type = 'CT' THEN (A.tot_apprvd_qty) END),0)CT_tot_apprvd_qty,IFNULL(SUM(CASE WHEN fg.type = 'CT' THEN (A.tot_rej_qty) END),0)CT_tot_rej
   ...
   $query = "SELECT sum(ord_qty) as tot_batch_qty,sum(ord_qty/12) as tot_batch_qty_doz,SUM(A.tot_apprvd_qty) as tot_apprvd_qty,SUM(A.tot_apprvd_qty/12) as tot_apprvd_qty_doz,((SUM(A.tot_apprvd_qty)/(sum(ord_qty))*100))as tot_apprvd_qty_per,SUM(A.tot_rej_qty) as tot_rej,SUM(A.tot_rej_qty/12) as tot_rej_doz,((SUM(A.tot_rej_qty)/sum(ord_qty))*100)as tot_rej_per,COUNT(jc.batch_no) as tot_batch_crd_issued,
